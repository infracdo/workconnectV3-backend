            ...
            if (result.get("status").equals("SUCCESS")) {
                if(authService.getRoles(authToken).contains("ROLE_USERADMIN_CREATE_USER")) {
                    ...
                    */
                    ...
                    String emailBody = mail_body.replace("user_placeholder", username);
                    emailBody = emailBody.replace("pass_placeholder", pass);
                    /*"Welcome " + username + "!" + "\r\n" +
    ...
    public ResponseEntity<Object> getRealmRoles(String auth) {
        ...
        if (result.get("status").equals("SUCCESS")) {
            if(authService.getRoles(auth).contains("ROLE_TENANT_ADMIN")) {
                ...
                // Get realm
                RealmResource realmResource = keycloak.realm(keycloak_relm);
                System.out.println("RESULT------------ "+realmResource.roles().get("GROUP_TENANT_USER").toRepresentation().getName());
                
                RoleRepresentation role_user = realmResource.roles().get("GROUP_TENANT_USER").toRepresentation();
                Map<Object,String> tenant_user = new HashMap<Object,String>();
                List<Map<Object,String>> tenant_user_role = new ArrayList<Map<Object,String>>();
                tenant_user.put("id", role_user.getId());
                tenant_user.put("name", role_user.getName());
                tenant_user_role.add(tenant_user);
                return new ResponseEntity<Object>(tenant_user_role, HttpStatus.OK);
            }else{
                Keycloak keycloak = KeycloakBuilder.builder() //
                    .serverUrl(keycloak_url.replace("\r\n","")+"auth") //
                    .realm(keycloak_relm_master.replace("\r\n","")) //
                ...
                HashMap<Object,String> role_holder = new HashMap<Object,String>();
                List<Map<Object,String>> filterRoles = new ArrayList<Map<Object,String>>();
                for(RoleRepresentation roleRepresentation: roles) {
                    
                    if(roleRepresentation.getName().contains("GROUP")){
                        role_holder.put("name", roleRepresentation.getName());
                        filterRoles.add(role_holder);
                        role_holder = new HashMap<Object,String>();
                    }
                    System.out.println("contatinate roles------------ "+filterRoles);
                };

                System.out.println("RESULT------------ "+realmResource.roles().list());
                return new ResponseEntity<Object>(filterRoles, HttpStatus.OK);
            }
        }else{
